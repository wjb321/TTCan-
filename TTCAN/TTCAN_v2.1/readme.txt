...
1.7.22
plug in encoder speed calculation part in side of the node2 and then node2 calculates the speed according to the number of rising edges then the speed info can be 
send via ttcan to node1, then node 1 will do some action regarding the car speed states, this is the draft version in V2.1, and keep debugging in v2.0 to test the consuming time
of 2 parts, first, time in speed calculation and state judge and state info sending. second, time consuming in ttcan message sending and pwm control