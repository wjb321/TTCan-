...
float THighFrequence = 1000.0; //reference frequency
float MFrequency_EncoderPulse = 1000; // car encoder frequency
float catchPeriod = 499.0; //in ms 
int main(void)
{
  ...
  TIM4_Int_Init(((1/ (THighFrequence ) * pow(10, 4)))-1,71);  //9
  TIM2_Int_Init(catchPeriod *10,7199);
  while(1)
    {
      key=KEY_Scan(0);	//得到键值
      if(key)
