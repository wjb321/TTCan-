...
SemaphoreHandle_t BinarySemaphore;//计数型信号量
    ...
    xSemaphoreTake(BinarySemaphore, portMAX_DELAY);
	  ...
	  for(times=0; times <2000000; times ++) // low priority is always taking the binary semaphore, high priority can not take it during this 
			{                                     // for loop so this make the priority flip
				taskYIELD(); // can be replaced by vTaskDelay();
				//Delay_us(2);
			}
		xSemaphoreGive(BinarySemaphore); // when the while finished, then free the BinarySemaphore, then high can take the resource and running
		LED0=!LED0;
