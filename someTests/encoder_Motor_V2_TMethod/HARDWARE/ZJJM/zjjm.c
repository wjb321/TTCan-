...
void calc_motor_rotate_speed()
{
	...
	if(circle_count ==5)
	{
     ...
     encoderNum = read_encoder() + 47* circle_count;
    /* 转速(1秒钟转多少圈)=单位时间内的计数值/总分辨率*时间系数 */
     rotateSpeed = (float)(encoderNum/(TOTAL_RESOLUTION*(Tmethod/pow(10, 4))));  //0.05 //0.23 
...
void TIM4_Mode_Config(int state)
{
    ...
    TIM_TimeBaseStructure.TIM_Period = 47;//7;//7;  //设定计数器重装值   TIMx_ARR = 1024*4-1 这是360线的  47
